...
// Set from SIGINT/SIGTERM so headless runs exit cleanly; the GLFW path has the
// window close for that.
static std::atomic<bool> g_headless_stop{false};
#include "param.h"
...
// Physics with no window.
//
// MuJoCo only needs GL for the viewer; stepping does not. Running without one
// matters in three places: a CI machine, the robot itself, and any X display
// without GLX (which is what a bare Xvfb or a nested server gives you — the
// GLFW path segfaults there rather than degrading).
//
// This is deliberately not PhysicsLoop: that one is driven by the UI's speed
// slider, pause state and drag perturbations, none of which exist here. What it
// shares is the thing that matters — stepping mj_step in real time, so the DDS
// bridge sees a clock that matches wall time.
void HeadlessLoop(const char *filename)
{
  char error[1000] = "";
  m = mj_loadXML(filename, nullptr, error, sizeof(error));
  if (!m)
  {
    std::cerr << "[headless] cannot load " << filename << ": " << error << std::endl;
    std::exit(1);
  }
  d = mj_makeData(m);
  mj_forward(m, d);
  ...
  std::cout << "[headless] " << filename << " loaded; stepping at "
            << 1.0 / m->opt.timestep << " Hz. Ctrl-C to quit." << std::endl;

  using Clock = std::chrono::steady_clock;
  auto next = Clock::now();
  const auto step = std::chrono::duration<double>(m->opt.timestep);
  while (!g_headless_stop.load(std::memory_order_relaxed))
  {
    // Elastic band, same as PhysicsLoop applies it under the UI.
    //
    // Not optional here: the G1 is limp in the controller's Passive state and
    // hits the floor within a second, long before anything can drive it to
    // FixStand — and FixStand cannot stand up from prone. Under the GUI a human
    // holds the robot on the band through that transition and releases it with
    // a key. Headless has no key, so `band_release_s` releases it on a timer.
    if (param::config.enable_elastic_band == 1 && elastic_band.enable_)
    {
      if (param::config.band_release_s > 0.0 && d->time >= param::config.band_release_s)
      {
        elastic_band.enable_ = false;
        std::cout << "[headless] elastic band released at t=" << d->time << " s"
                  << std::endl;
    ...
    // Behind schedule (a slow step, or the process was descheduled): give up
    // catching up rather than spiralling, and resync. Silently accumulating lag
    // would make the bridge's timestamps drift away from wall time.
  ...
  std::cout << "[headless] stopped after " << d->time << " s of sim time" << std::endl;
}
...
int main(int argc, char **argv)
{
  ...
  {
    // No window: the DDS bridge still comes up, so the sim is fully usable.
    // This is the mode for CI, for the robot, and for any X display without
    // GLX.
    std::signal(SIGINT,  [](int) { g_headless_stop = true; });
    std::signal(SIGTERM, [](int) { g_headless_stop = true; });
    std::thread unitree_thread_headless(UnitreeSdk2BridgeThread, nullptr);
    unitree_thread_headless.detach();
    HeadlessLoop(param::config.robot_scene.c_str());
    return 0;
  }
  ...
  // Side panels off unless asked for (config.yaml or --ui). They cost about a
  // third of the window, and during a sim2sim run what is worth watching is on
  // the bus, not in them.
  sim->ui0_enable = param::config.show_left_ui;
  sim->ui1_enable = param::config.show_right_ui;
