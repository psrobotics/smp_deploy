...
public:
    ...
    : mj_model_(model), mj_data_(data)
    {
        ...
        if(param::config.use_joystick == 1) {
            ...
            } else if(param::config.joystick_type == "lcm") {
                // Gamepad over LCM rather than a kernel device — this is what
                // makes the controller's FSM scriptable, and so sim2sim
                // automatable. A real pad still works: bridge /dev/input onto
                // the same channel (smp_hw's scripts/joy_bridge.py).
                joystick = std::make_shared<sim2sim::LcmJoystick>(
                    param::config.joystick_lcm_channel, param::config.joystick_lcm_url);
            } else {
