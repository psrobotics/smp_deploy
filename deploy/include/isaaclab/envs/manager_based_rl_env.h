...
public:
    ...
    void reset()
    {
        ...
        if (alg) alg->reset();  // recurrent runners drop their state too
    }
    ...
    void step()
    {
        ...
        if (dashboard_enabled) {
            ...
            for (const auto& kv : obs) {
                obs_dim += static_cast<uint32_t>(kv.second.size());
            }
            dashboard.set_obs_dim(obs_dim);
        }
        auto action = alg->act(obs);
        if (dashboard_enabled) {
            dashboard.set_action_dim(static_cast<uint32_t>(action.size()));
        }
