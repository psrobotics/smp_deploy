...
public:
    ...
    // Called by ManagerBasedRLEnv::reset(). Stateless runners (plain ORT
    // policies) ignore it; a recurrent runner must drop its state here, or a
    // re-entered FSM state replays stale plans against a fresh robot pose.
    virtual void reset() {}
