...
struct ArticulationData
{
    ...
    // Where an external twist source plugs in. Set use_vel_cmd_override and
    // write [vx, vy, wz] before each step; velocity_commands then reads from
    // here instead of the pad, clamped to the env config's ranges either way.
    // Nothing in this repository sets it.
    bool  use_vel_cmd_override  = false;
    float vel_cmd_override[3]   = {0.0f, 0.0f, 0.0f};
};
